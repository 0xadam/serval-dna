...
void _rhizome_manifest_set_sender(struct __sourceloc __whence, rhizome_manifest *m, const sid_t *sidp)
{
  if (sidp) {
    ...
    m->sender = *sidp;
    m->has_sender = 1;
    m->has_csender = 0; // do I really need to set this?
    m->finalised = 0;
  } else
...
void _rhizome_manifest_set_sender_concealed(struct __sourceloc __whence, rhizome_manifest *m, keyring_file *keyring)
{
   ...
   unsigned char crypted_sid[SID_SIZE];
    ...
    const char *v = rhizome_manifest_set(m, "csender", (char *)crypted_sid);
    assert(v);
    bcopy(crypted_sid, m->csender, SID_SIZE);

    const char *w = rhizome_manifest_set(m, "csenderPublic", alloca_tohex_sid_t(concealed_sender));
    assert(w);
